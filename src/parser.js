...
function collectPosts(data, config) {
  ...
  const posts = getItemsOfType(data, "post")
    ...
    .map((post) => {
      ...
      if (postLocation) {
        postData.frontmatter.location = postLocation;
      }
...
function getPostLocation(post) {
  const postmeta = post.postmeta.find(
    (postmeta) => postmeta.meta_key[0] === "location"
  );
  const locationmeta = postmeta ? postmeta.meta_value[0] : undefined;

  if (!locationmeta) {
    return undefined;
  }

  const [title, latlng] = locationmeta.split("|");
...
function getPostFormat(post) {
  const formatmeta = post.category.find(
    (postcategory) => postcategory.$.domain === "post_format"
  );
  const format = formatmeta ? formatmeta._.toLowerCase() : "standard";
  return format;
}

function getPostAuthor(post) {
  return post.creator[0] === "admin" ? "David" : "Carman";
}
  ...
  images.forEach((image) => {
    ...
    if (post) {
      if (image.id === post.meta.coverImageId) {
        ...
        post.frontmatter.ogImage = shared.getFilenameFromUrl(image.url);
      }
      ...
      if (!post.meta.imageUrls.includes(image.url)) {
        ...
        post.frontmatter.images.push(shared.getFilenameFromUrl(image.url));
      }
    }
  });
}
